...
Notice that the tracer object is no longer returned. Consult the documentation to see [all possible parameters](https://godoc.org/github.com/stlimtat/dd-trace-go/ddtrace/tracer#StartOption) to the `Start` call.
...
Starting spans is now possible with [functional options](https://godoc.org/github.com/stlimtat/dd-trace-go/ddtrace/tracer#StartSpanOption). Which means that all span properties (or none) can be set when starting a span dynamically. Before:
...
```
You will notice that the new tracer also introduces the concept of [SpanContext](https://godoc.org/github.com/stlimtat/dd-trace-go/ddtrace#SpanContext), which is different from Go's context and is used to carry information needed to spawn children of a specific span and can be propagated cross-process. To learn more about distributed tracing check the package-level [documentation](https://godoc.org/github.com/stlimtat/dd-trace-go/ddtrace/tracer#ChildOf) of the `tracer` package.
...
* package level documentation of the [`tracer` package](https://godoc.org/github.com/stlimtat/dd-trace-go/ddtrace/tracer) for a better overview.
